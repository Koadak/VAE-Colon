...
# Sinkhorn Distance for EMD approximation
def sinkhorn_distance(z_true, z_pred, epsilon=0.01, num_iters=50):
    """
    Approximate Earth Mover's Distance using the Sinkhorn-Knopp algorithm.
    - z_true: (batch_size, latent_dim) - The latent representations from real samples.
    - z_pred: (batch_size, latent_dim) - The latent representations from generated samples.
    """
    cost_matrix = tf.norm(tf.expand_dims(z_true, 1) - tf.expand_dims(z_pred, 0), axis=-1)

    # Compute softmin for Sinkhorn iterations
    u = tf.zeros_like(cost_matrix)
    v = tf.zeros_like(cost_matrix)

    for _ in range(num_iters):
        u = -epsilon * tf.math.log(tf.reduce_mean(tf.exp(-cost_matrix / epsilon + v[:, None]), axis=1))
        v = -epsilon * tf.math.log(tf.reduce_mean(tf.exp(-cost_matrix / epsilon + u[None, :]), axis=0))

    sinkhorn_dist = tf.reduce_mean(cost_matrix * tf.exp(-cost_matrix / epsilon + u[:, None] + v[None, :]))
    return sinkhorn_dist

# Custom VAE model class with EMD loss
class VAE(Model):
    ...
    def train_step(self, data):
        images, _ = data  # Only use images, labels are not needed
        with tf.GradientTape() as tape:
            z_mean, z_log_var, z = self.encoder(images)
            reconstructed = self.decoder(z)

            # Reconstruction Loss (MSE)
            recon_loss = self.mse_loss(images, reconstructed)
            ...
            # Total loss (Reconstruction + EMD)
            total_loss = recon_loss + emd_loss

        grads = tape.gradient(total_loss, self.trainable_variables)
        self.optimizer.apply_gradients(zip(grads, self.trainable_variables))
        return {"loss": total_loss, "reconstruction_loss": recon_loss, "emd_loss": emd_loss}

# Function to create the complete VAE
def create_vae():
